 ...
 *
 * - Análise de complexidade:
 * - Nas linhas 213, 214 e 227 são criados objetos que tomam tempo constante. O(1)
 * - A linha 217 será executada N + 1 vezes, dependendo do número de pontos recebidos dentro de points,
 * logo, terá complexidade O(N).
 * - As linha 219 até 222 farão as comparações de custo O(1), mas como estão dentro de um loop, logo
 ...
 *
 * - Análise de complexidade:
 * - Nas linhas 254 a 256 são criados e calculados variáveis de tempo constante. O(1)
 *
 * Portanto a função é limitada pela complexidade constante O(1).
 *
 * Corretude:
 ...
 *
 * - Análise de complexidade:
 * - Nas linhas 279 a 282 são criados e calculados variáveis de tempo constante. O(1)
 *
 * Portanto a função é limitada pela complexidade constante O(1).
 *
 * - Corretude:
 ...
 *
 * - Análise de complexidade:
 * - A linha 304 executa N + 1 vezes, sendo N o tamanho da lista. Tem complexidade O(N).
 * - A linha 305 está limitada pelo loop dos pontos. Portanto executa linearmente
 ...
 *
 * - Análise de complexidade:
 * - As linhas 360 e 361 executam em tempo constante. O(1)
 * - A linha 362 executa N + 1 vezes, sendo N o tamanho da lista. Tem complexidade O(N).
 * - A linha 365 chama uma função que é de complexidade constante O(1), porém
 * está dentro do loop com complexidade O(N).
 * - Da linha 365 até 368, são executadas dentro do loop com complexidade O(N).
 * - A linha 372 que retorna o valor é de complexidade constante O(1).
 *
 * A função é limitada pela complexidade linear O(N).
 *
 * Corretude:
 ...
 *
 * - Análise de complexidade:
 * - A linhas 424 e 428 executam em tempo constante. O(1)
 * - A linha 429 executa 1 vez, mas sua complexidade é linear em função da capacity enviada.
 * Portanto tem complexidade da ordem O(N)
 * - A linha 431 executa N + 1 vezes, sendo N o tamanho da lista. Tem complexidade O(N).
 * - As linha 433 a 434 chamam funções e fazem comparações de complexidade constante O(1),
 * como estão dentro de um loop, tem com complexidade O(N).
 * - A linha 438 que retorna o valor é de complexidade constante O(1).
 *
 ...
 *
 * - A linha 649 executa uma comparação de tempo constante O(1).
 * - A linha 653 realiza uma chamada de função de complexidade O(N).
 * - As linhas 657 e 659 executam em tempo constante, pois apenas alocam variáveis. O(1)
 * - A linha 658 e 660 realiza a chamada de uma função com complexidade O(N).
 * - As linhas 664 e 670 realizam a chamada recursiva, que será melhor analisado posteriormente. O(N * lg(N)) em média.
 * - A linha 668 adiciona ao vetor, que é de custo constante O(1).
 *
 ...
 *
 * - Análise de complexidade:
 * - A linha 710 executa a função get_extreme_points, que é de ordem linear, portanto O(N).
 * - As linha 716 e 730 realiza uma operação constante de inserção em lista encadeada, portanto O(1).
 * - As linha 726 e 732 realizam chamadas de quick_hull_rec, que possuem complexidade no pior caso de O(N^2)
 * e no melhor caso e caso médio de O(N*lgN).
 * - As linha 721 e 722 chamam funções que são de complexidade O(N).
 *
 ...
 *
 * - Análise de complexidade:
 * Linhas 760 a 765 fazem chamadas constantes, O(1)
 * Linhas 766 chama a função read_input, que é da ordem de O(N)
 * Linhas 769 e 774 executam operações de relógio constantes, O(1)
 * Linhas 771 chama a função quick_hull, que pode ser O(N * lgN) ou O(N^2)
 * Linhas 776 chama a função write_output, que é da ordem de O(N)
 * Linhas 779 a 782 executam operações e chamadas de função independentes
